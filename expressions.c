...
int get_operator_precedence(TokenType token) {
    if (token == PLUS_OPERATOR || token == MINUS_OPERATOR) return 1;
    if (token == ASSIGNMENT_OPERATOR || 
        ...
        token == GREATER_THAN_EQUAL_OPERATOR) return 2;
    if (token == MULTIPLICATION_OPERATOR || token == DIVISION_OPERATOR) return 3;
    return 0;
}
